...
async function main() {
  ...
  const SPACEBOTS = await ethers.getContractFactory("SPACEBOTS");

  const name = "SPACEBOTS"
  const symbol = "SPACEBOTS"
  const initBaseURI = "https://spacebots.iotabots.io/"


  const url: string = "https://raw.githubusercontent.com/iotabots/save-the-bots/main/spacebots_drop.txt";
  ...
  try {
    ...
    if (response.data) {
      ...
      const spacebots = await SPACEBOTS.deploy(name, symbol, initBaseURI, chunk_arry[0]);

      await spacebots.deployed();

      console.log("SPACEBOTS deployed to:", spacebots.address);
      ...
      for (var chunk = 1; chunk < chunk_arry.length; chunk++) {
        ...
        while (minting) {
            ...
            try {
                ...
                let airdrop = await spacebots.airdrop(chunk_arry[chunk]);
